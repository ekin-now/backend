...
type AuthReq = { user: { sub: string; role: UserRole } };
...
export class PostController {
  ...
  getFeed(@Request() req: AuthReq, @Query() query: PostFeedQueryDto) {
    return this.postService.getFeed(req.user.sub, query);
  }

  @ApiCreatedResponse({ type: PostResponseDto })
  @Post()
  create(@Request() req: AuthReq, @Body() dto: CreatePostDto) {
    return this.postService.create(dto, req.user.sub);
  }
  ...
  remove(@Request() req: AuthReq, @Param('id') id: string) {
    return this.postService.remove(id, req.user.sub, req.user.role);
  }
